...
import { checkSettingsArtifacts, generateSettingsArtifacts } from "./artifacts.js";
import { discoverSettingsProjects } from "./projects.js";
...
export async function runCli(args, io = processIo) {
    ...
    if (discovered.value.length === 0) {
        io.stderr(line(`No package.json with a piExtensionSettings manifest was found under ${parsed.root}`));
        return 1;
    }
    if (parsed.command === "generate") {
        for (const project of discovered.value) {
            ...
            for (const path of generated.value.changedPaths)
                io.stdout(line(`updated ${path}`));
        }
        return 0;
    }
    let stale = false;
