import { cloneJson, isJsonObject, type JsonObject, type JsonValue } from "./json-value.ts";

function mergeValue(current: JsonValue | undefined, next: JsonValue): JsonValue {
    if (!isJsonObject(current) || !isJsonObject(next)) return cloneJson(next);

    const merged: Record<string, JsonValue> = {};
    for (const [key, value] of Object.entries(current)) merged[key] = cloneJson(value);
...
export function mergeSettings(current: JsonObject, next: JsonObject): JsonObject {
